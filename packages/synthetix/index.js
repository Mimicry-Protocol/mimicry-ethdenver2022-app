...
const defaults = {
	...
	COLLATERAL_MANAGER: {
		...
		SHORTS: ['mBTC', 'mETH'],
		MAX_DEBT: w3utils.toWei('75000000'), // 75 million mUSD
...
const getFeeds = ({ network, path, fs, deploymentPath, useOvm = false } = {}) => {
	...
	return Object.entries(feeds).reduce((memo, [asset, entry]) => {
		memo[asset] = Object.assign(
			...
			// Note: ETH still used as a rate for Depot, can remove the below once the Depot uses mETH rate or is
	...
	return [
		...
		synths
			...
			.map(synth => ({
				...
				address: (targets[`Proxy${synth.name === 'mUSD' ? 'ERC20mUSD' : synth.name}`] || {})
					.address,
				index: synth.index,
				decimals: 18,
				feed: synth.feed,
