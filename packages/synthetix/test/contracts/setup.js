...
const setupContract = async ({
	...
	const defaultArgs = {
		...
		Synth: [
			...
			tryGetProperty({ property: 'name', otherwise: 'Synthetic mUSD' }),
			tryGetProperty({ property: 'symbol', otherwise: 'mUSD' }),
			owner,
			tryGetProperty({ property: 'currencyKey', otherwise: toBytes32('mUSD') }),
...
const setupAllContracts = async ({
	...
	const baseContracts = [
		...
		},
		{
			...
			mocks: [],
			deps: ['AddressResolver', 'EtherWrapper', 'WETH', 'SynthmETH'],
		},
		{
	...
	if (returnObj['ExchangeRates']) {
		...
		await setupPriceAggregators(returnObj['ExchangeRates'], owner, [MIME]);
		await updateAggregatorRates(returnObj['ExchangeRates'], [MIME], [toUnit('0.2')]);
	}
