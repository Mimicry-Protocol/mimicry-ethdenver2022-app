...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    /// @return the last exchange rate of the synth to mUSD
    ...
    function _updateMIMEIssuedDebtOnExchange(bytes32[2] memory currencyKeys, uint[2] memory currencyRates) internal {
        ...
        if (!includesmUSD) {
            keys[2] = mUSD; // And we'll also update mUSD to account for any fees if it wasn't one of the exchanged currencies
    ...
    function _dynamicFeeRateForCurrency(bytes32 currencyKey, DynamicFeeConfig memory config)
        ...
        returns (uint dynamicFee, bool tooVolatile)
    {
        // no dynamic dynamicFee for mUSD or too few rounds
        if (currencyKey == mUSD || config.rounds <= 1) {
            return (0, false);
        }
