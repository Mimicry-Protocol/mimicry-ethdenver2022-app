...
contract ExchangerWithFeeRecAlternatives is MinimalProxyFactory, Exchanger {
    ...
    function _exchangeAtomically(
        ...
        // One of src/dest synth must be mUSD (checked below for gas optimization reasons)
        require(
            !exchangeRates().synthTooVolatileForAtomicExchange(
                sourceCurrencyKey == mUSD ? destinationCurrencyKey : sourceCurrencyKey
            ),
        ...
        // Ensure src/dest synth is mUSD and determine mUSD value of exchange
        uint sourcemUSDValue;
        if (sourceCurrencyKey == mUSD) {
            // Use after-settled amount as this is amount converted (not sourceAmount)
            sourcemUSDValue = sourceAmountAfterSettlement;
        } else if (destinationCurrencyKey == mUSD) {
            // In this case the systemConvertedAmount would be the fee-free mUSD value of the source synth
            sourcemUSDValue = systemConvertedAmount;
        } else {
            revert("Src/dest synth must be mUSD");
        }

        // Check and update atomic volume limit
        _checkAndUpdateAtomicVolume(sourcemUSDValue);
        ...
        if (fee > 0) {
            ...
            fee = exchangeRates().effectiveValue(destinationCurrencyKey, fee, mUSD);

            // Remit the fee in mUSDs
            issuer().synths(mUSD).issue(feePool().FEE_ADDRESS(), fee);

            // Tell the fee pool about this
            feePool().recordFeePaid(fee);
        }

        // Note: As of this point, `fee` is denominated in mUSD.
    ...
    function _checkAndUpdateAtomicVolume(uint sourcemUSDValue) internal {
        uint currentVolume =
            uint(lastAtomicVolume.time) == block.timestamp
                ? uint(lastAtomicVolume.volume).add(sourcemUSDValue)
                : sourcemUSDValue;
